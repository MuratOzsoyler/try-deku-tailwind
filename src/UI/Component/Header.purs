...
import Deku.Core (NutWith, fixed)
...
header = do
  ...
  pure Deku.do
    ...
    Pursx.header ~~
      ...
      , link1Attrs: empty
      , menuDivAttrs: DA.selfT_ \div -> do
          let parent = HTMLDiv.toParentNode div
          mbul <- runMaybeT do
            Console.debug "in runMaybeT"
            elem <- MaybeT $ ParentNode.querySelector (QuerySelector "ul") parent
            Console.debug "found ul"
            MaybeT $ pure $ HTMLUList.fromElement elem
          Console.debug $ "runMaybeT result isNothing=" <> show (isNothing mbul)
          setMenuElem mbul
      , menuAttrs: empty -- oneOf
      --   [ DA.selfT_ \(elem :: Web.HTMLUListElement) -> do
      --   [ DA.selfT_ \(elem :: Web.HTMLElement) -> do
      --       Console.log "SelfT"
